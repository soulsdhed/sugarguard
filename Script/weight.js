...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 저장 버튼
    const saveButton = document.getElementById("weight-save");
    // 시간 요소
    ...
    if (data.wl_id) {
        try {
            const response = await fetchGetWithRetry("/api/weight-logs", {
                params: {
                    wl_id: data.wl_id,
                },
                withCredentials: true,
            });
            const responseData = response.data.data.weight_logs[0];
            // 시간 정보
    ...
    // 기록 데이터에 받아온 데이터 입력
    if (data.weight != null) weightInput.value = Number(data.weight).toFixed(2);
