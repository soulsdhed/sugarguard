...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addRecord(recordType, imgUrl, recordClass, item = null) {
    ...
    if (recordClass == "exercise-record") {
      ...
      if (item != null) {
        ...
        // 시간 부분 개선
        if (["공복", "취침 전", "실시간"].includes(item.record_type)) {
          timeString = `${item.record_type} ${convertToKSTAndFormat(
            item.record_time
          )}`;
        } else {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                if (match) {
                  ...
                  try {
                    const response = await fetchDeleteWithRetry(apiLink, data, {
                      withCredentials: true,
                    });
  ...
  async function generateCalendar(selectedDate) {
    // 안쪽 로딩 만들 것
    // 로딩 시작
    document.getElementById("inner-loading-screen").style.display = "flex";
    ...
    currentMonth = (selectedDate.getMonth() + 1).toString().padStart(2, "0");
    ...
    try {
      ...
      responseBloodSugar.data.data.blood_sugar_logs.forEach((item) => {
        addRecord(
          "혈당 기록",
          "https://res.cloudinary.com/difzc7bsf/image/upload/v1721719247/blood_h0b2io.png",
          "sugar-record",
      ...
      responseBloodPressure.data.data.blood_pressure_logs.forEach((item) => {
        addRecord(
          "혈압 기록",
          "https://res.cloudinary.com/difzc7bsf/image/upload/v1721983336/%EC%A0%9C%EB%AA%A9%EC%9D%84-%EC%9E%85%EB%A0%A5%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94_-009_basbm8.png",
          "blood-pressure-record",
      ...
      // 체중 기록
      const responseWeight = await fetchGetWithRetry("/api/weight-logs", {
        params: {
          startDate: `${currentYear}-${currentMonth}-${currentDay}`,
          endDate: `${currentYear}-${currentMonth}-${currentDay}`,
        },
      ...
      // 운동 기록
      const responseExcercise = await fetchGetWithRetry("/api/exercise-logs", {
        params: {
          startDate: `${currentYear}-${currentMonth}-${currentDay}`,
          endDate: `${currentYear}-${currentMonth}-${currentDay}`,
        },
    ...
    // 안쪽로딩 만들 것
    // 로딩화면 제거
    document.getElementById("inner-loading-screen").style.display = "none";
  }
